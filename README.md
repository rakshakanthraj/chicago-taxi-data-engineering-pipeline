...
- Used for implementing SCD Type 2 historical tracking using dbt snapshots.

macros/

- Contains reusable dbt logic functions used in transformations.
...
- No data loss detected across layers
- No duplicate records after transformation
- Revenue reconciled successfully between Silver and Gold layers
- Pipeline output validated and analytics-ready
...
Run the Bronze ingestion notebook in Databricks to load raw taxi data into Delta tables.

Silver Layer

Run the Silver transformation notebook to perform data cleaning, schema enforcement, null handling, and deduplication.

Snapshot Layer

Execute dbt snapshots to capture historical changes and maintain SCD Type 2 records.

dbt snapshot

Gold Layer

Execute dbt models to build the Star Schema and analytical tables.

dbt run

Validation

Run reconciliation and audit queries in Databricks to validate record counts and revenue consistency between Silver and Gold layers.

Analytics

Query Gold layer fact and dimension tables for reporting and business analysis.

---

📌 Future Enhancements

- Integrate Apache Airflow or Databricks Workflows for pipeline orchestration.
- Implement CI/CD using GitHub Actions to automate dbt testing and deployment.
- Migrate manual validation queries to an automated dbt testing framework.
- Build Power BI or Tableau dashboards on top of the Gold layer for business reporting.
